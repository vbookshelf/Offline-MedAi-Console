...
# --- MODIFIED --- Removed in-memory store for uploaded images.
# Images will now be saved to a persistent folder.
...
# --- ADDED --- Folder to store persistent agent profile pictures
AVATARS_FOLDER = "agent_avatars"

# --- ADDED --- Folder to store persistent user uploads for chat history
PERSISTENT_UPLOADS_FOLDER = "persistent_uploads"
...
# --- MODIFIED --- List of models to exclude from TTS
TTS_EXCLUSION_LIST = [
	"hf.co/unsloth/medgemma-1.5-4b-it-GGUF:BF16",
]
...
# Address CORS vulnerability by not setting cors_allowed_origins="*"
...
<head>
    ...
    <style>
        ...
        .agent-icon-image {
            width: 4rem;
            height: 4rem;
            border-radius: 0.75rem; /* rounded-xl to match the theme */
            object-fit: cover;
            border-width: 4px;
            border-color: #1e293b; /* slate-800 */
        }
        ...
        .hidden-reasoning ul, .hidden-reasoning ol {
            margin-left: 1.2rem;
            margin-bottom: 0.5rem;
        }
...
<body class="flex h-screen overflow-hidden text-slate-800">
    ...
    <div class="main-flex-container flex flex-1 overflow-hidden relative max-w-7xl mx-auto transition-all duration-300">
        ...
        <aside id="agent-sidebar" class="w-full md:w-80 lg:w-96 p-4 bg-slate-800 border-r border-slate-700 text-slate-100 overflow-y-auto flex-shrink-0 absolute md:relative h-full z-20 md:z-10 transform -translate-x-full md:translate-x-0">
			...
			<div class="mb-4">
				...
				<!--
				<label for="model-selector" id="model-selector-label" class="block text-sm font-medium text-slate-300 mb-2">Tool: <span class="text-indigo-400 font-semibold text-lg">Ai Assistant</span></label>
				-->
				
                <button id="toggle-toolbox-btn" class="sidebar-toggle-btn w-full text-left py-2.5 px-4 bg-slate-700/50 text-slate-200 rounded-lg hover:bg-slate-700 transition-colors duration-200 flex items-center justify-between">
                    <span>Toolbox</span>
                ...
                <div id="toolbox-container" class="hidden mt-2 p-3 bg-slate-900/50 rounded-lg space-y-4">
                    <div>
            ...
            <div class="mb-4">
				<label for="model-selector" id="model-selector-label" class="block text-sm font-medium text-slate-300 mb-2">Tool: <span class="text-indigo-400 font-semibold text-lg">Ai Assistant</span></label>
                <label class="block text-sm font-medium text-slate-300 mb-2">Tool Settings:</label>
                <div class="relative">
                    <select id="model-selector" class="bg-slate-700/50 w-full py-2.5 px-4 rounded-lg text-white focus:outline-none transition-all duration-200 appearance-none hover:bg-slate-700">
                        {% for model in model_list %}
                        <option value="{{ model }}" {% if model == current_model %}selected{% endif %} class="bg-slate-800 text-white font-semibold">{{ model }}</option>
                        {% endfor %}
                    </select>
			...
			<div class="mb-4">
                <button id="toggle-params-btn" class="sidebar-toggle-btn w-full text-left py-2.5 px-4 bg-slate-700/50 text-slate-200 rounded-lg hover:bg-slate-700 transition-colors duration-200 flex items-center justify-between">
            ...
            <div class="mb-10">
                <button id="toggle-voice-btn" class="sidebar-toggle-btn w-full text-left py-2.5 px-4 bg-slate-700/50 text-slate-200 rounded-lg hover:bg-slate-700 transition-colors duration-200 flex items-center justify-between">
			...
			<div class="mb-4">
                <button id="toggle-webcam-btn" class="sidebar-toggle-btn w-full text-left py-2.5 px-4 bg-slate-700/50 text-slate-200 rounded-lg hover:bg-slate-700 transition-colors duration-200 flex items-center justify-between">
        ...
        <main class="flex-1 flex flex-col bg-white relative">
            ...
            <!-- Panel Toggle Button -->
            <button id="toggle-panel-btn" class="hidden md:block absolute top-14 left-1 z-30 p-1 rounded-md hover:bg-slate-300 text-slate-600 transition-colors" title="Toggle Sidebar">
			...
			<header class="md:hidden flex items-center justify-between p-3 bg-white border-b border-slate-200 shadow-sm">
			    ...
			    <h2 id="mobile-header-title" class="font-bold text-lg">MedAi Console</h2>
			    <div class="w-8"></div>
			</header>
            ...
            <div id="tab-header" class="hidden flex w-full flex-shrink-0 p-1 pr-3 bg-white/80 backdrop-blur-sm border-b border-slate-200 shadow-sm items-center justify-between">
                ...
                <div class="flex items-center ml-4 flex-shrink-0">
                    <button id="new-chat-btn" class="text-sm font-semibold text-slate-500 hover:text-slate-400 p-2 rounded-lg flex items-center gap-2">
                        New Chat
                    </button>
                    <button id="global-history-btn" class="ml-2 text-sm font-semibold text-slate-500 hover:text-slate-400 p-2 rounded-lg flex items-center gap-2">
                        History
                    </button>
                </div>
            </div>
            ...
            <div id="tab-content" class="flex-1 flex flex-col p-2 sm:p-4 overflow-hidden">
                <div id="initial-message" class="flex-1 flex items-center justify-center text-center text-slate-500">
                    <div>
                        <span class="text-7xl text-slate-300 mb-2">MedAi Console</span>
                        <p class="text-2xl font-semibold mb-2 mt-3">Offline-First Ai</p>
						<!--
						<p class="text-base mb-2 mt-3">Select a tool from the toolbox to get started.</p>
						-->
						</div>
                </div>
            </div>
    ...
    <div id="agent-editor-modal" class="hidden fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div class="bg-white rounded-lg p-6 w-full max-w-2xl shadow-xl transform transition-all duration-300 scale-95 opacity-0">
            <h2 id="agent-modal-title" class="text-2xl font-bold text-slate-800 mb-6">Create an Ai Tool</h2>
			
			<p class="text-xs text-slate-400 mt-1 pb-2">
                        Privacy note: The agent system instruction, profile picture and your chat history are saved on this device. All saved data is stored in the app folder. You can delete your entire chat history by deleting the conversations.json file. All agent data can be deleted by deleting the agents.json file and deleting the agents_avatars folder. 
             </p>
            ...
            <form id="agent-editor-form">
                ...
                <div class="flex flex-col md:flex-row gap-6">
                    ...
                    <div class="flex-shrink-0 flex flex-col items-center md:items-start">
                        <label class="block text-sm font-medium text-slate-700 mb-2 text-center md:text-left">Profile Picture</label>
                        <img id="agent-avatar-preview" class="h-28 w-28 rounded-xl object-cover bg-slate-200 mb-2 border-2 border-slate-300" src="data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%2394a3b8'%3e%3cpath fill-rule='evenodd' d='M18.685 19.097A9.723 9.723 0 0021.75 12c0-5.385-4.365-9.75-9.75-9.75S2.25 6.615 2.25 12a9.723 9.723 0 003.065 7.097A9.716 9.716 0 0012 21.75a9.716 9.716 0 006.685-2.653zm-12.54-1.285A7.486 7.486 0 0112 15a7.486 7.486 0 015.855 2.812A8.224 8.224 0 0112 20.25a8.224 8.224 0 01-5.855-2.438zM15.75 9a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z' clip-rule='evenodd' /%3e%3c/svg%3e" alt="Avatar Preview">
                        <input type="file" id="agent-avatar-input" class="hidden" accept="image/*">
                        <button type="button" id="change-avatar-btn" class="w-full py-2 px-3 bg-slate-100 hover:bg-slate-200 text-sm font-semibold text-slate-700 rounded-lg">Change</button>
                    </div>
                ...
                <!-- Persona Textarea (Full Width Below) -->
                <div class="mt-4">
    ...
    <script type="module">
        ...
        function renderAgents() {
            ...
            agents.forEach((agent, index) => {
                ...
                if (agent.avatar_url) {
                    ...
                    img.src = `${agent.avatar_url}?t=${new Date().getTime()}`;
                    img.className = 'agent-icon-image';
                    iconContainer.appendChild(img);
                } else {
                    const iconDiv = document.createElement('div');
                    iconDiv.className = 'agent-icon';
                    iconDiv.style.backgroundColor = agent.color;
                    const initialSpan = document.createElement('span');
                    initialSpan.textContent = agent.name.charAt(0);
                    iconDiv.appendChild(initialSpan);
                ...
                nameH3.className = 'font-semibold text-slate-50 text-lg truncate';
                ...
                if (index === 0) moveUpBtn.classList.add('invisible');
                if (index === agents.length - 1) moveDownBtn.classList.add('invisible');
        ...
        function openChatTab(agent) {
            ...
            if (modelSelectorLabel) {
                 modelSelectorLabel.innerHTML = `Tool: <span class="text-indigo-400 font-semibold text-lg">${agent.name}</span>`;
            }
        ...
        async function closeChatTab(agentId) {
            ...
            if (remainingTabKeys.length > 0) {
                ...
                const modelSelectorLabel = document.getElementById('model-selector-label');
                if (modelSelectorLabel) {
                     modelSelectorLabel.innerHTML = `Tool: <span class="text-indigo-400 text-lg font-semibold">Ai Assistant</span>`;
                }
		...
		function createChatView(agent) {
            ...
            stopBtn.onclick = () => socket.emit('stop_generation');
            ...
            newUploads.forEach(uploadItem => {
                ...
                .then(async response => {
                    if (!response.ok) {
                         ...
                         let errorMessage = (errorData && errorData.error) ? errorData.error : `Server error: ${response.status}`;
        ...
        // --- MODIFIED --- This function now renders a larger avatar in the welcome message.
        function renderChatHistory(agentId) {
            ...
            if (history.length === 0) {
                 ...
                 agentIconContainer.className = 'mx-auto mb-4';

                 if (agent.avatar_url) {
                    // Use specific classes to override the default smaller icon size
                    agentIconContainer.innerHTML = `<img src="${agent.avatar_url}?t=${new Date().getTime()}" class="w-40 h-40 rounded-xl object-cover mx-auto border-4 border-white shadow-lg">`;
                 } else {
                    const agentIcon = document.createElement('div');
                    // Add size and font classes for the larger default icon
                    agentIcon.className = 'agent-icon w-24 h-24 mx-auto border-4 border-white shadow-lg text-4xl';
                 ...
                 welcomeDiv.appendChild(agentIconContainer);
                ...
                let messagesToShow = showFullHistory ? history : history.slice(-2);
                if (!showFullHistory && history.length > 1 && history[history.length - 1].role !== 'assistant') {
		...
		function renderMessage(agentId, msg) {
		    ...
		    const imagePreviews = part.image_previews || [];
            const imageFilenames = part.image_filenames || [];
		    ...
		    if (rawText.trim().length > 0 || !isUser) {
		        if (!isUser && thinkingContent) {
                    ...
                    thinkingEl.className = "thinking-bubble bg-slate-200 text-slate-600 italic rounded-xl mb-2 cursor-pointer";
                    
                    // Create the summary (Clickable header)
                    const summaryEl = document.createElement("summary");
                    summaryEl.className = "p-2 list-none select-none";
                    summaryEl.textContent = "View reasoning";

                    // When the dropdown is open, clicking anywhere on it will close it.
                    thinkingEl.addEventListener('click', function(e) {
                        // This listener only acts when the dropdown is already open.
                        if (this.open) {
                            // We prevent the default toggle behavior of the summary element,
                            // ensuring the dropdown reliably closes.
                            e.preventDefault();
                            this.open = false;
                        }
                    });
                    ...
                    // Add 'markdown-content' class so it gets tables, lists, and bold styling
                    contentDiv.className = "hidden-reasoning mt-2 text-sm bg-slate-100 p-2 rounded markdown-content";
                    
                    // Parse markdown to HTML
                    contentDiv.innerHTML = marked.parse(thinkingContent);
                    
                    // Add syntax highlighting and copy buttons to code blocks in reasoning
                    enhanceCodeBlocks(contentDiv);

                    // Assemble the details element
                    thinkingEl.appendChild(summaryEl);
                    thinkingEl.appendChild(contentDiv);
		...
		function openCreateAgentModal() {
            ...
            document.getElementById('agent-avatar-preview').src = placeholderAvatar;
            document.getElementById('agent-avatar-input').value = '';
        ...
        function openEditAgentModal(agent) {
            ...
            if (agent.avatar_url) {
                avatarPreview.src = `${agent.avatar_url}?t=${new Date().getTime()}`;
            } else {
                avatarPreview.src = placeholderAvatar;
            }
            document.getElementById('agent-avatar-input').value = '';
        ...
        async function handleSaveAgent(e) {
            ...
            const agentData = { name, title, persona, type, color: '#4f46e5' };
            ...
            if (agentId) {
                ...
                agentData.id = name.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '') + '-' + Date.now();
            }
            ...
            try {
                ...
                if (agentId) {
                    ...
                    if (index !== -1) {
                        const updatedAgent = { ...agents[index], ...savedAgent };
                        agents[index] = updatedAgent;

                        // If a tab for the edited agent is open, refresh its content
                        if (activeChats[agentId]) {
                            activeChats[agentId].agent = updatedAgent; // Update agent data in state

                            // Update the tab button text
                            const tabBtn = document.getElementById(`tab-btn-${agentId}`);
                            if (tabBtn) {
                                tabBtn.querySelector('span:first-child').textContent = updatedAgent.name;
                            }

                            // If this is the current agent, update the main tool label
                            if (currentAgentId === agentId) {
                                const modelSelectorLabel = document.getElementById('model-selector-label');
                                if (modelSelectorLabel) {
                                     modelSelectorLabel.innerHTML = `Tool: <span class="text-indigo-400 font-semibold text-lg">${updatedAgent.name}</span>`;
                                }
                            }

                            // Re-render chat history to reflect changes (e.g., in the welcome message)
                            renderChatHistory(agentId);
                        }
                    }
                } else {
        ...
        function renderSavedChatsList(agentId) {
            ...
            chats.forEach(chat => {
                ...
                buttonsDiv.innerHTML = `
                    <button class="edit-history-btn text-slate-500 hover:text-slate-400 p-1 opacity-0 transition-opacity" title="Edit title">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg>
                    </button>
                    <button class="delete-history-btn text-slate-500 hover:text-slate-400 p-1 opacity-0 transition-opacity" title="Delete chat">
        ...
        function toggleListening(agentId) {
            ...
            // Check if we are currently listening based on the class state
            const wasListening = micBtn.classList.contains('listening');
            ...
            if (wasListening) {
                ...
                // 3. Stop Audio Playback (if any is playing or queued)
                // This ensures that clicking the button kills the voice output immediately.
                stopAudioPlayback();
            } else {
                // --- STARTING ---
                
                // Optional: Prevent starting if AI is busy typing, 
                // but allow if it's just audio playing (so we can interrupt)
                if (isTyping && !isAudioPlaying) return;

                // If audio is currently playing, stop it so we can listen
                if (isAudioPlaying) {
                    stopAudioPlayback();
                }
        ...
        function setupSocketListeners() {
            ...
            socket.on('chat_end', async (data) => {
                ...
                if (chat && chat.history.length > 0) {
                    const lastMsg = chat.history[chat.history.length - 1];
                    // --- MODIFICATION START ---
                    // Update history with both the final message and the thinking content
                    lastMsg.parts[0].text = data.final_message;
                    lastMsg.parts[0].thinking = data.thinking_content;
                    
                    // Re-render the specific message to include the thinking bubble
                    renderChatHistory(currentAgentId); 
                    // --- MODIFICATION END ---
        ...
        function setupEventListeners() {
            ...
            toggleToolboxBtn.addEventListener('click', () => {
                toolboxContainer.classList.toggle('hidden');
            });
            ...
            document.getElementById('new-chat-btn').addEventListener('click', () => {
                ...
                if (chat) {
                    ...
                    if (chat.uploadQueue.length > 0) {
                        chat.uploadQueue.forEach(item => URL.revokeObjectURL(item.preview));
                        chat.uploadQueue = [];
		...
		document.addEventListener('DOMContentLoaded', async () => {
            ...
            if (defaultAgent) {
                const agentToOpen = agents.find(a => a.id === defaultAgent.id);
                if (agentToOpen) {
                    openChatTab(agentToOpen);
                }
            }
...
DEFAULT_AGENT = {
    ...
    "title": "A helpful medical assistant",
...
def create_agent():
    ...
    if 'agent_data' not in request.form:
        return jsonify({"error": "Missing agent data"}), 400
        
    new_agent_data = json.loads(request.form['agent_data'])

    if not all(k in new_agent_data for k in ['id', 'name', 'title', 'persona', 'type']):
        return jsonify({"error": "Missing agent data fields"}), 400
    
    new_agent_data['isDefault'] = False
    new_agent_data['avatar_url'] = None

    if 'profile_picture' in request.files:
        file = request.files['profile_picture']
        if file and file.filename != '' and allowed_file(file.filename):
            filename = secure_filename(f"{new_agent_data['id']}_{file.filename}")
            file.save(os.path.join(AVATARS_FOLDER, filename))
            new_agent_data['avatar_url'] = f"/avatars/{filename}"
...
def update_agent(agent_id):
    ...
    updated_data = json.loads(request.form['agent_data'])
                ...
                if file and file.filename != '' and allowed_file(file.filename):
                    if agent.get('avatar_url'):
                        try:
                            ...
                            print(f"[ERROR] Could not delete old avatar: {e}", file=sys.stderr)
                    
                    filename = secure_filename(f"{agent_id}_{file.filename}")
                    file.save(os.path.join(AVATARS_FOLDER, filename))
                    updated_data['avatar_url'] = f"/avatars/{filename}"
...
def delete_agent(agent_id):
    ...
    if agent_to_delete.get('avatar_url'):
        try:
            filename = agent_to_delete['avatar_url'].split('/')[-1]
            filepath = os.path.join(AVATARS_FOLDER, filename)
            if os.path.exists(filepath):
                os.remove(filepath)
                print(f"[CLEANUP] Deleted avatar for agent {agent_id}: {filename}")
        except Exception as e:
            print(f"[ERROR] Could not delete avatar for agent {agent_id}: {e}", file=sys.stderr)
...
def index():
    all_agents = load_agents()
    default_agent = next((agent for agent in all_agents if agent.get("isDefault")), None)
    user_settings = load_settings()
    response = Response(render_template_string(
        HTML_TEMPLATE, 
        model_list=model_list, 
        current_model=MODEL_NAME, 
        saved_settings=user_settings,
...
# --- MODIFIED --- Removed cleanup of in-memory store on disconnect.
...
# --- MODIFIED --- This function now reads files from disk instead of an in-memory store.
...
def handle_chat_message(data):
    ...
    if model in TTS_EXCLUSION_LIST:
        tts_enabled = "Off"
        print(f"[INFO] TTS disabled for model in exclusion list: {model}")
    ...
    for msg in history:
        ...
        if image_ids:
            ...
            for image_filename in image_ids:
                image_path = os.path.join(PERSISTENT_UPLOADS_FOLDER, image_filename)
                
                if os.path.exists(image_path):
                    extension = image_filename.rsplit('.', 1)[1].lower()
                    base64_data, error = None, None

                    try:
                        with open(image_path, "rb") as f:
                            if extension == 'pdf':
                                base64_data, error = process_pdf_to_base64(f, image_filename)
                            else:
                                base64_data, error = process_image_to_base64(f)

                        if error:
                            print(f"[ERROR] Could not process file {image_filename}: {error}", file=sys.stderr)
                            continue
                        
                        if base64_data:
                            images_base64_list.extend(base64_data)

                    except Exception as e:
                        print(f"[ERROR] Failed to read or process file {image_filename}: {e}", file=sys.stderr)

                else:
                    print(f"[WARNING] Image file '{image_filename}' not found in persistent store for SID {request.sid}")
            
            if images_base64_list:
                ollama_msg["images"] = images_base64_list
    ...
    try:
        ...
        for chunk in response_stream:
            ...
            if tts_enabled == "On":
                ...
                # Exclude reasoning from being spoken
                end_marker = "<unused95>"
                if end_marker in text_to_speak:
                    text_to_speak = text_to_speak.split(end_marker, 1)[1]
        ...
        start_marker = "<unused94>thought"
        end_marker = "<unused95>"
        
        reasoning_content = ""
        final_message = full_response.strip()

        if start_marker in full_response and end_marker in full_response:
            try:
                start_index = full_response.find(start_marker) + len(start_marker)
                end_index = full_response.find(end_marker)
                reasoning_content = full_response[start_index:end_index].strip()
                final_message = full_response[end_index + len(end_marker):].strip()
            except Exception as e:
                print(f"[ERROR] Failed to parse reasoning from response: {e}", file=sys.stderr)
                reasoning_content = ""
                final_message = full_response.strip()

        socketio.emit('chat_end', {
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PERSISTENT_UPLOADS_FOLDER):
        os.makedirs(PERSISTENT_UPLOADS_FOLDER)
